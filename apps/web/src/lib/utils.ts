...
function resolveBaseServerUrl(url?: string | undefined): string {
  ...
  const bootstrapUrl = resolvePrimaryEnvironmentBootstrapUrl();
  if (isNonEmptyString(bootstrapUrl)) {
    return bootstrapUrl;
  }

  return firstNonEmptyString(import.meta.env.VITE_WS_URL, window.location.origin);
}
